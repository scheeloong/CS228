...
def get_pixels_sampled_from_p_x_joint_z1_z2():
  ...
  for pixel in range(28 * 28):
    ans = np.append(ans, sampleBinary(
        get_p_xk_cond_z1_z2(sampledZ1, sampledZ2, pixel)))
  '''
...
def get_conditional_expectation(data):
  ...
  for i, image in enumerate(data):
    ...
    probability_of_image = math.exp(fastLogProb(image, probs, priors))
    average_z1 = 0.0
    average_z2 = 0.0
    counter = 0
    for z1 in disc_z1:
      for z2 in disc_z2:
        ...
        prior_prob = priors[counter]
        p_x_given_z = probs[counter]
        total_prob = math.exp(probCalculation(mask, p_x_given_z) + math.log(prior_prob))
        p_of_z_given_i = total_prob / probability_of_image

        average_z1 += z1*p_of_z_given_i
        average_z2 += z2*p_of_z_given_i
        counter += 1

    ZZ1.append(average_z1/25.)
    ZZ2.append(average_z2/25.)

  return (np.array(ZZ1), np.array(ZZ2))
...
def probCalculation(mask, prob):
	# print (np.sum(np.log(np.select([mask, np.invert(mask)], [prob,
	# 1.-prob]))) + 500.)
	return (np.sum(np.log(np.select([mask, np.invert(mask)], [prob, 1. - prob]))))
...
def fastLogProb(image, probs, priors):
	...
	for i, prob in enumerate(probs):
		total += math.exp(probCalculation(mask, prob) + 500. + math.log(priors[i]))
		# print (total)
...
def q7():
  ...
  plt.figure() 
  plt.scatter(mean_z1, mean_z2, c=labels, cmap = cm.jet)
  #plt.scatter([0,1], [2,3])
  plt.colorbar()
  plt.grid()
